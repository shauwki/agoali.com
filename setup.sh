...
backup_n8n() {
    ...
    docker exec -u node agoa_n8n n8n export:workflow --all --output="${container_path}/workflows.json"
    echo "--> Workflows succesvol geëxporteerd."

    echo "-> Credentials (versleuteld) exporteren..."
    docker exec -u node agoa_n8n n8n export:credentials --all --output="${container_path}/credentials.json"
...
restore_n8n() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        docker exec -u node agoa_n8n n8n import:credentials --input="${container_path}/credentials.json"

        echo "-> Workflows importeren..."
        docker exec -u node agoa_n8n n8n import:workflow --input="${container_path}/workflows.json"
